...
const PUBLIC_API = "api.oryxcrm.com";
...
function convertEndpointToOperation(params) {
  ...
  const statusCode =
    ...
    parseInt(endpoint.responses[0].headers.status.split(" ")[0], 10);
  ...
  const requiredPreviewHeaders = endpoint.previews.filter(
    preview => preview.required
  );
  const defaultAcceptHeader = requiredPreviewHeaders.length
    ? requiredPreviewHeaders
        .map(header => `application/vnd.github.${header.name}-preview+json`)
        .join(",")
    : "application/vnd.github.v3+json";
  const acceptHeaderParam = {
    name: "accept",
    description: requiredPreviewHeaders.length
  ...
  const codeSampleParams = { scope, endpoint, baseUrl };
...
function toShellExample({ endpoint, scope, baseUrl }) {
  const path = urlTemplate
    .parse(endpoint.path.replace(/:(\w+)/, "{$1}"))
    .expand(PARAMETER_EXAMPLES);
...
function isGitHubCloudOnly(endpoint) {
  ...
  const hasGitHubEnterpriseNote = /is available to organizations with GitHub Enterprise Cloud/.test(
        ...
        if (addition.items.type === "object") {
          ...
          addition.items.properties = paramsToProperties(
            params
              ...
              .map(p =>
                Object.assign({}, p, { name: p.name.substr(prefix.length) })
              )
          );
...
function toRequestBodyParameterName(endpoint) {
  const paramSentAsRoot = endpoint.params.find(
    param => param.mapTo === "input"
  );
...
function toRequestBodySchema(endpoint) {
  ...
  if (paramSentAsRoot) {
    const description =
      paramSentAsRoot.description || `${paramSentAsRoot.name} parameter`;
